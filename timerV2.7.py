...
def hbox_avviso_gen(entry_callback1, entry_callback2, button_callback, container):
	def entry_avviso_callback(entry):
		if entry.get_text() == "":	entry_callback1(entry)
		else:						entry_callback2(entry)
	hbox_avviso = gtk.HBox(False, 0)
	hbox_avviso.show()
	container.pack_start(hbox_avviso, False, False, 0)
	
	entry_avviso = gtk.Entry()
	hbox_avviso.pack_start(entry_avviso, False, False, 5)
	entry_avviso.handler_id = entry_avviso.connect("activate", entry_avviso_callback)
	entry_avviso.set_tooltip_text("in minuti")
	entry_avviso.show()
	
	button_avviso = gtk.Button("+")
	button_avviso.handler_id = button_avviso.connect("clicked", button_callback)
...
class Timer:
	def __init__(self):
		...
		self.entry_tempo.set_tooltip_text("in minuti, s = 1 secondo\n"
										  "es: un minuto e mezzo = 1 + 30*s")
		...
		hbox_avviso = hbox_avviso_gen(self.avvia_callback, self.aggiungi_avviso_callback, self.aggiungi_avviso_callback, self.vbox_avvisi)
		
		###	LayoutTime
		self.box_time = gtk.VBox(False, 0)
		self.tutto.add(self.box_time)
		
		hbox_stop = gtk.HBox(False, 0)
		hbox_stop.show()
		self.box_time.pack_start(hbox_stop, False, False, 0)
	...
	def aggiungi_avviso_callback(self, widget):
		hbox_avviso_pre = self.vbox_avvisi.get_children()[1]
		entry_avviso_pre, button_avviso_pre = hbox_avviso_pre.get_children()
		button_avviso_pre.set_label(" - ")
		button_avviso_pre.disconnect(button_avviso_pre.handler_id)
		button_avviso_pre.connect("clicked", self.rimuovi_avviso_callback, hbox_avviso_pre)
		
		hbox_avviso = hbox_avviso_gen(self.avvia_callback, self.aggiungi_avviso_callback, self.aggiungi_avviso_callback, self.vbox_avvisi)
		entry_avviso = hbox_avviso.get_children()[0]
		entry_avviso.grab_focus()
		self.vbox_avvisi.reorder_child(hbox_avviso, 1)
		
	def rimuovi_avviso_callback(self, widget, hbox_avviso):
	...
	def avvia_callback(self, widget):
		if self.entry_tempo.get_text() > "":
			...
			for hbox in self.vbox_avvisi.get_children()[1:]:
				avviso = hbox.get_children()[0].get_text()
				if avviso > "" and eval(avviso) <= eval(self.entry_tempo.get_text()): self.lista_avvisi.append(eval(avviso))
			
			self.window.set_resizable(True)
			self.window.resize(self.window.get_size()[0], 1)
			self.window.set_title("Timer "+self.nome)
			self.box_data.hide()
			self.box_time.show()
			self.conteggio = True
			self.update()
